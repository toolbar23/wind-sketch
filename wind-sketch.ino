...
double readAnalogRotationValue() {  //returns mastRotate value when called. This is in degrees, and corresponds to the current value of the Honeywell sensor
  ...
  // Define Constants
  const int lowset = 1200;//4095 * 0.1;
  const int highset = 3150;//4095 * 0.9;
...
//delay(250);
 ...
 int newValue = honeywellSensor.reading(adc1);  // calculate the moving average
  ...
  if (newValue < 3500) {  //writes value to oldsensor if below highset threshold
    oldValue = newValue;
  }
  ...
  double mastRotate = map(oldValue, lowset, highset, -45, 45);  //maps 12 bit number to degrees of rotation
  debug(" angle ") ;
   debug(mastRotate);
